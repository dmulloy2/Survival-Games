...
public class UpdateChecker {
	...
	public void check(Player player, Plugin p) {
		...
		if (! SettingsManager.getInstance().getConfig().getBoolean("check-for-updates")) {
			SurvivalGames.debug("Update checking disabled in the config!");
			return;
		}
		
		SurvivalGames.debug("Checking for updates!");
		...
		try {
            ...
            if (b) {
            	...
            	player.sendMessage(ChatColor.DARK_BLUE+"--------------------------------------");
            	SurvivalGames.$("An update is available: " + in[1] + "!");
            }
        } catch(Exception e) {
