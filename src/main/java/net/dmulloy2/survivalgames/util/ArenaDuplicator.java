...
public class ArenaDuplicator
{
	...
	public ArenaDuplicator(SurvivalGames plugin)
	{
		this.plugin = plugin;
	}
	...
	public void startDupe(Vector v1, Vector v2)
	{
		...
		int maxx = plugin.getGameManager().getGame(1).getArena().getMax().getBlockX();
		...
		background = new background(Math.abs(v2.getBlockX() - v1.getBlockX()) * Math.abs(v2.getBlockY() - v1.getBlockY())
				* Math.abs(v1.getBlockZ() - v2.getBlockZ()));
		background.start();
	...
	{
		World w = plugin.getSettingsManager().getGameWorld(1);
		Vector min;
		Vector max;
		int xoff;
		int zoff;
					...
					for (int z = min.getBlockZ(); z < max.getBlockZ(); z++)
					{
						...
						try
						{
							if (l1.getBlock().getTypeId() != l2.getBlock().getTypeId())
							{
								...
								// chunk.a(l2.getBlockX() & 15, l2.getBlockY() ,
								// l2.getBlockZ() & 15,
								// l1.getBlock().getTypeId(),
								// l1.getBlock().getData());
